...
function sumAmountStolen(
  ops: ProvinceHistoryPoint["thieveryOpsTaken"],
): number {
  return ops
    .filter((op) => ROB_OPS.has(op.op))
    .reduce((sum, op) => sum + (op.amountStolen ?? 0), 0);
}
...
function buildRows(
  ...
  const bucketMs =
    history.length <= TARGET_POINTS
      ? 0 // no bucketing
      : (NICE_BUCKETS_MS.find((b) => b >= rawBucketMs) ??
        NICE_BUCKETS_MS[NICE_BUCKETS_MS.length - 1]);
  ...
  if (!bucketMs) {
    return {
      ...
      rows: history.map((point) => {
        ...
        const otherOpsCount = point.thieveryOpsTaken.filter(
          (op) => !ROB_OPS.has(op.op),
        ).length;
        if (otherOpsCount > 0) row.thieveryOpsCount = otherOpsCount;
        if (point.sorceryOpsTaken.length > 0)
          row.sorceryOpsCount = point.sorceryOpsTaken.length;
        return row;
      }),
    };
  }
  ...
  const rows = [...groups.entries()]
    ...
    .map(([bucketStart, points]) => {
      const iso = new Date(bucketStart)
        .toISOString()
        .replace("T", " ")
        .replace(".000Z", "");
      const meta: MetaMap = {};
      for (const p of points) {
        for (const [key, m] of Object.entries(p.meta)) {
          const existing =
            meta[key] ?? (meta[key] = { sources: [], savedBy: [] });
          for (const s of m!.sources)
            if (!existing.sources.includes(s)) existing.sources.push(s);
          for (const s of m!.savedBy)
            if (!existing.savedBy.includes(s)) existing.savedBy.push(s);
        }
      }
      ...
      if (stolen > 0) row.thieveryAmountStolen = stolen;
      const otherOpsCount = thieveryOpsTaken.filter(
        (op) => !ROB_OPS.has(op.op),
      ).length;
      if (otherOpsCount > 0) row.thieveryOpsCount = otherOpsCount;
      if (sorceryOpsTaken.length > 0)
        row.sorceryOpsCount = sorceryOpsTaken.length;
      return row;
    });
...
function ChartTooltip({
  ...
  const point = (payload[0] as Payload<number, string> | undefined)?.payload as
    | ChartRow
    | undefined;
  if (!point) return null;

  const visible = (payload as Payload<number, string>[]).filter(
    (p) => METRIC_BY_KEY.has(p.dataKey as MetricKey) && p.value != null,
  );
  const attacks = hideAttacks ? [] : point.attacksTaken;
  const totalAcresTaken = attacks.reduce(
    (sum, a) => sum + (a.acresTaken ?? 0),
    0,
  );
  const totalLosses = attacks.reduce(
    (sum, a) =>
      sum + (a.killed ?? 0) + (a.imprisoned ?? 0) + (a.massacred ?? 0),
    0,
  );
  const allThieveryOps = point.thieveryOpsTaken;
  const robOps = hideThievery
    ? []
    : allThieveryOps.filter((op) => ROB_OPS.has(op.op));
  const sabotageOps = hideSabotageOps
    ? []
    : allThieveryOps.filter((op) => !ROB_OPS.has(op.op));
  const sorceryOps = hideSorcery ? [] : point.sorceryOpsTaken;
  const totalStolen = robOps.reduce(
    (sum, op) => sum + (op.amountStolen ?? 0),
    0,
  );
  const stolenByType = new Map<
    string,
    { stolen: number; thievesLost: number; successes: number; failures: number }
  >();
  for (const op of robOps) {
    const e = stolenByType.get(op.op) ?? {
      stolen: 0,
      thievesLost: 0,
      successes: 0,
      failures: 0,
    ...
    if (op.outcome === "success") e.successes++;
    else e.failures++;
    stolenByType.set(op.op, e);
  }
  const thieveryContributors = [
    ...new Set(robOps.map((op) => op.attackerName)),
  ];
  const thieverySuccesses = robOps.filter(
  ...
  return (
    <div
      className="rounded border border-gray-700 bg-gray-900 p-2 text-xs shadow-lg"
      style={{ minWidth: 220, maxWidth }}
    >
      ...
      <div className="space-y-0.5">
        {attacks.length > 0 && (
          <div className="mb-1 rounded border border-rose-900/70 bg-rose-950/30 p-1.5">
            <div className="flex items-center justify-between gap-3 text-rose-200">
              <span>
                {attacks.length} attack{attacks.length === 1 ? "" : "s"} taken
              </span>
              <span className="tabular-nums text-gray-400">
                {totalAcresTaken > 0 && <>{totalAcresTaken} acres · </>}
                {formatNum(totalLosses)} losses
              </span>
            </div>
                ...
                return (
                  <Fragment key={`${attack.receivedAt}:${i}`}>
                    <span className="text-gray-400">
                      {attackTypeShort(attack.attackType)}
                    </span>
        ...
        {robOps.length > 0 && (
          <div className="mb-1 rounded border border-amber-900/70 bg-amber-950/30 p-1.5">
            <div className="flex items-center justify-between gap-3 text-amber-200">
              <span>
                ...
                <span className="ml-1.5 text-gray-500">
                  {thieverySuccesses}✓
                  {thieveryFailures > 0 && <> {thieveryFailures}✗</>}
                </span>
              </span>
              {totalStolen > 0 && (
                <span className="tabular-nums">
                  {formatNum(totalStolen)} stolen
                </span>
              )}
            </div>
            <div className="mt-1 grid grid-cols-[auto_1fr_auto_auto] gap-x-2 gap-y-0.5 text-gray-600">
              ...
              <span>Stolen</span>
              <span>✓/✗</span>
              <span>Thieves Lost</span>
              {(["vaults", "granaries", "towers"] as const)
                .filter((op) => stolenByType.has(op))
                .map((op) => {
                  const { stolen, thievesLost, successes, failures } =
                    stolenByType.get(op)!;
                  const resource =
                    op === "vaults"
                      ? "gold"
                  ...
                  return (
                    <Fragment key={op}>
                      ...
                      <span className="tabular-nums text-gray-500">
                        {stolen > 0 ? `${formatNum(stolen)} ${resource}` : "—"}
                      </span>
                      <span className="tabular-nums text-gray-500">
                        {successes}✓{failures > 0 ? ` ${failures}✗` : ""}
                      </span>
                      <span className="tabular-nums text-gray-500">
                        {thievesLost > 0 ? formatNum(thievesLost) : "—"}
                      </span>
                    </Fragment>
                  );
                })}
            ...
            <div className="mt-0.5 text-gray-500">
              ...
              {thieveryContributors.slice(0, 3).join(", ")}
              {thieveryContributors.length > 3
                ? ` +${thieveryContributors.length - 3} more`
                : ""}
            </div>
          </div>
        )}
        {sabotageOps.length > 0 && (
          <div className="mb-1 rounded border border-violet-900/70 bg-violet-950/30 p-1.5">
            <div className="text-violet-200">
              {sabotageOps.length} sabotage op
              {sabotageOps.length === 1 ? "" : "s"}
            </div>
            <div className="mt-1 grid grid-cols-[auto_1fr_auto_auto] gap-x-2 gap-y-0.5 text-gray-600">
              <span>Op</span>
              <span>Result</span>
              ...
              {sabotageOps.map((op, i) => {
                ...
                if (op.outcome === "failure") {
                  ...
                  switch (op.op) {
                    ...
                    case "incite_riots":
                      result =
                        op.effectDuration != null
                          ? `Rioting ${op.effectDuration} days`
                          : "Incite Riots";
                      break;
                    case "sabotage_wizards":
                      result =
                        op.effectDuration === 0
                          ? "No lasting effect"
                          : op.effectDuration != null
                            ? `Sabotage ${op.effectDuration} days`
                            : "Sabotage Wizards";
                      break;
                    case "arson":
                    case "greater_arson":
                      result =
                        op.acresBurned === 0
                          ? "No buildings found"
                          : op.acresBurned != null
                            ? `Burned ${formatNum(op.acresBurned)} acres`
                            : "Arson";
                      break;
                    case "destabilize_guilds":
                      result =
                        op.effectDuration != null
                          ? `Disrupted ${op.effectDuration} days`
                          : "Destabilize Guilds";
                      break;
                    case "bribe_thieves":
                ...
                return (
                  <Fragment key={`${op.receivedAt}:${i}`}>
                    ...
                    <span className="tabular-nums text-gray-500">
                      {op.thievesLost > 0 ? formatNum(op.thievesLost) : "—"}
                    </span>
                  </Fragment>
                );
              })}
            ...
            {(() => {
              const contributors = [
                ...new Set(sabotageOps.map((op) => op.attackerName)),
              ];
              return (
                <div className="mt-0.5 text-gray-500">
                  <span className="text-gray-600">By: </span>
        ...
        {sorceryOps.length > 0 && (
          <div className="mb-1 rounded border border-teal-900/70 bg-teal-950/30 p-1.5">
            ...
            <div className="mt-1 grid grid-cols-[1fr_auto_auto] gap-x-2 gap-y-0.5 text-gray-600">
              ...
              {sorceryOps.slice(0, 8).map((op, i) => (
                <Fragment key={`${op.receivedAt}:${i}`}>
                  ...
                  <span className="tabular-nums text-gray-500">
                    {op.durationDays != null ? `${op.durationDays}d` : "—"}
                  </span>
                </Fragment>
              ))}
            </div>
            {sorceryOps.length > 8 && (
              <div className="mt-0.5 text-gray-600">
                +{sorceryOps.length - 8} more
              </div>
            )}
            {(() => {
              const contributors = [
                ...new Set(sorceryOps.map((op) => op.casterName)),
              ];
              return (
                <div className="mt-0.5 text-gray-500">
                  <span className="text-gray-600">By: </span>
            ...
            <div key={String(p.dataKey)}>
              ...
              {m && (m.sources.length > 0 || m.savedBy.length > 0) && (
                <div className="pl-1 text-gray-600">
                  {m.sources.length > 0 && (
                    <span>{truncateList(m.sources)}</span>
                  )}
                  {m.sources.length > 0 && m.savedBy.length > 0 && (
                    <span> · </span>
                  )}
                  {m.savedBy.length > 0 && (
                    <span>{truncateList(m.savedBy)}</span>
                  )}
                </div>
              )}
            </div>
...
export function ProvinceHistoryChart({
  ...
  const [hidden, setHidden] = useState<Set<MetricKey>>(
    new Set(
      METRICS.filter((m) => !["networth", "land"].includes(m.key)).map(
        (m) => m.key,
      ),
    ),
  );
  ...
  const displayedHistory = useMemo(
    () => filterHistoryByDateRange(history, fromMs, toMs),
    [history, fromMs, toMs],
  );
  ...
  return (
    <section
      ref={containerRef}
      className="mt-6 rounded-lg border border-gray-800 bg-gray-900/50 p-4"
    >
            ...
            >
              ...
              <Tooltip
                ...
                content={(props) => (
                  <ChartTooltip
                    ...
                    tz={tz}
                    hideAttacks={hideAttacks}
                    hideThievery={hideThievery}
                    hideSabotageOps={hideSabotageOps}
                    hideSorcery={hideSorcery}
              ...
              {METRICS.map((m) => (
                <Line
                  ...
                  dot={
                    hoveredLine === m.key
                      ? { r: 3, fill: m.color, strokeWidth: 0 }
                      : false
                  }
