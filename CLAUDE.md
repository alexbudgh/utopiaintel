...
- Spy/thievery intel commonly arrives as `/wol/game/thievery?...&o=SPY_ON_*`, but the same `o` param also appears on `/wol/game/province_operations/<x>/<y>/<id>` (a general-purpose page that also covers combat spells, war room, and other actions). Both paths have sitter variants (`/wol/sit/game/...`). Detection must inspect the `o` query param — only recognized spy/thief ops are routed; unrecognized ops on `province_operations` fall through to null.
