...
test("estimatePop — maxPop: null when survey missing", () => {
  const { maxPop, needsForMax } = estimatePop(
    makeInputs({
      housing_effect: 10,
      sciences_age: T1,
    }),
...
test("estimatePop — currentPop: null when SoT missing", () => {
  const { currentPop, needsForCurrent } = estimatePop(
    makeInputs({
...
test("estimatePop — wizardsEstimated: false when wizards directly known", () => {
  const { wizardsEstimated } = estimatePop(
    makeInputs({
      peasants: 1000,
      soldiers: 0,
      off_specs: 0,
...
test("estimatePop — wizardsEstimated: true when wizards inferred from NW residual", () => {
  ...
  const { wizardsEstimated, currentPop } = estimatePop(
    makeInputs({
      networth: 300000,
      land: 1000,
      race: "Human",
  ...
  assert.ok(
    currentPop != null,
    "currentPop should be non-null when NW residual estimate succeeds",
  );
});
