...
export function estimatePop(p: PopInputs): PopEstimate {
  ...
  if (hasSurvey && hasHousing && surveyAndSciSameTick) {
    ...
    const rawCap =
      (p.buildings_built ?? 0) * 25 +
      (p.buildings_in_progress ?? 0) * 25 +
      barren * 15 +
      homes * 10;
    const raceFactor =
      p.race && RACE_POP_FACTOR[p.race] ? RACE_POP_FACTOR[p.race] : 1.0;
    const housingMult = 1 + (p.housing_effect ?? 0) / 100;
    const honorBonus =
      p.honor_title && HONOR_POP_BONUS[p.honor_title]
        ? HONOR_POP_BONUS[p.honor_title]
        : 0;
    const honorEffectMod =
      p.personality && PERSONALITY_HONOR_EFFECT_MOD[p.personality]
        ? PERSONALITY_HONOR_EFFECT_MOD[p.personality]
        : 1.0;
  ...
  const hasTroops =
    ...
    p.elites != null;
  const hasTraining =
    p.training_off_specs != null ||
    p.training_def_specs != null ||
    p.training_elites != null ||
