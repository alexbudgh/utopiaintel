...
const INVASION_RE = new RegExp(
  `^${PROV_REF} invaded ${PROV_REF} and captured (${INT}) acres`,
);
// Unknown province variants — must be checked before general patterns
const INVASION_UNKNOWN_RE = new RegExp(
  `^An unknown province from ([^(]+?)\\s*${KDLOC} invaded ${PROV_REF} and captured (${INT}) acres`,
);
const AMBUSH_ARMIES_UNKNOWN_RE = new RegExp(
  `^An unknown province from ([^(]+?)\\s*${KDLOC} ambushed armies from ${PROV_REF} and took (${INT}) acres`,
);
// Both "ambushed armies from Y and took N acres" and "recaptured N acres [of land] from Y"
// are the Ambush attack type in Utopia.
const AMBUSH_ARMIES_RE = new RegExp(
  `^${PROV_REF} ambushed armies from ${PROV_REF} and took (${INT}) acres`,
);
const AMBUSH_RE = new RegExp(
  `^${PROV_REF} recaptured (${INT}) acres(?:\\s+of\\s+land)?\\s+from ${PROV_REF}`,
);
const RAZE_RE = new RegExp(`^${PROV_REF} razed (${INT}) acres of ${PROV_REF}`);
const RAZE_INVADED_RE = new RegExp(
  `^${PROV_REF} invaded ${PROV_REF} and razed (${INT}) acres`,
);
...
const MARCH_RE = new RegExp(
  `^${PROV_REF} captured (${INT}) acres of land from ${PROV_REF}`,
);
// Unknown province march: "An unknown province from X captured N acres of land from Y"
const MARCH_UNKNOWN_RE = new RegExp(
  `^An unknown province from ([^(]+?)\\s*${KDLOC} captured (${INT}) acres of land from ${PROV_REF}`,
);
// Unknown province failed: standard and intra-kingdom war variant
const FAILED_RE = new RegExp(
  `^(?:In intra-kingdom war )?${UNKNOWN_PROV_RE.source} attempted to invade ${PROV_REF}`,
);
// Known attacker failed: "X attempted to invade Y" or "X attempted an invasion of Y"
const FAILED_KNOWN_RE = new RegExp(
  `^${PROV_REF} attempted (?:to invade|an invasion of) ${PROV_REF}`,
);
// Dragon arrived against us from news (different from SoT ravage)
const DRAGON_ARRIVED_RE = new RegExp(
  `^A (\\w+) Dragon, ([^,]+), from ([^(]+?)\\s*${KDLOC} has begun ravaging our lands!`,
);
// Dragon completed and launched by us: "X has completed our dragon, Name, and it sets flight to ravage Y (X:Y)!"
const DRAGON_LAUNCHED_RE = new RegExp(
  `^(.+?) has completed our dragon, ([^,]+), and it sets flight to ravage ([^(]+?)\\s*${KDLOC}!`,
);
// Dragon slain
const DRAGON_SLAIN_RE =
  /^(.+?) has slain the dragon, ([^,]+), ravaging our lands!/;
// Ritual started
...
const WAR_DECLARED_RE = new RegExp(
  `^We have declared WAR on ([^(]+?)\\s*${KDLOC}!`,
);
...
const CEASEFIRE_ACCEPTED_RE = new RegExp(
  `^([^(]+?)\\s*${KDLOC} has accepted our ceasefire proposal!`,
);
const CEASEFIRE_BROKEN_RE = new RegExp(
  `^([^(]+?)\\s*${KDLOC} has broken their ceasefire agreement with us!`,
);
const CEASEFIRE_WITHDRAWN_RE = new RegExp(
  `^We have withdrawn our ceasefire proposal to ([^(]+?)\\s*${KDLOC}`,
);
const DRAGON_BY_US_RE = new RegExp(
  `^Our kingdom has begun the (\\w+) Dragon project, ([^,]+), targeted at ([^(]+?)\\s*${KDLOC}`,
);
const DRAGON_AGAINST_US_RE = new RegExp(
  `^([^(]+?)\\s*${KDLOC} has begun the (\\w+) Dragon project, ([^,]+), against us!`,
);

function classifyEvent(
  text: string,
): Omit<KingdomNewsEvent, "gameDate" | "rawText"> {
...
const STOLEN_FROM_RE = new RegExp(
  `stolen the last 2 month's of kingdom news from [^(]+${KDLOC}`,
);
const CRYSTAL_EYE_TARGET_RE = new RegExp(
  `gleaned a glimpse into the last 2 month's of kingdom news from [^(]+${KDLOC}`,
);
...
export function parseKingdomNews(text: string): KingdomNewsData | null {
  ...
  const targetKingdom =
    targetKingdomMatch?.[1] ??
    stolenMatch?.[1] ??
    crystalEyeTargetMatch?.[1] ??
    null;
