...
function parseArgs(args: string[]): Options {
  ...
  const checkpointSources = [
    opts.tailFromPath,
    opts.resumeFromMysqlWatermarks,
  ].filter(Boolean);
  if (checkpointSources.length > 1) {
    throw new Error(
      "--tail-from and --resume-from-mysql-watermarks are mutually exclusive",
    );
  }
...
let checkpoint: Checkpoint | null = options.tailFromPath
  ? readCheckpointFile(options.tailFromPath)
  : null;
...
function selectRows<T>(
  ...
  return sqlite
    .prepare(`SELECT * FROM ${table} WHERE id > ? ORDER BY ${orderBy}`)
    .all(minId) as T[];
}
...
function captureCheckpoint(sqlite: Database.Database): Checkpoint {
  return Object.fromEntries(
    ID_TABLES.map((table) => {
      const row = sqlite
        .prepare(`SELECT COALESCE(MAX(id), 0) AS max_id FROM ${table}`)
...
async function insertOne(
  ...
  const [res] = (await conn.execute(sql, params as any)) as [
    ResultSetHeader,
    unknown,
  ];
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [rows] = (await conn.execute(conflictSql, conflictParams as any)) as [
    { id: number }[],
    unknown,
  ];
  return rows[0].id;
...
async function resolveOne(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [rows] = (await conn.execute(sql, params as any)) as [
    { id: number }[],
    unknown,
  ];
  if (!rows[0])
    throw new Error(`Unable to resolve existing MySQL row for SQL: ${sql}`);
  return rows[0].id;
}
...
async function readMysqlWatermarks(conn: PoolConnection): Promise<Checkpoint> {
  ...
  const [rows] = (await conn.query(
    "SELECT source_table, source_max_id FROM sqlite_migration_watermarks",
  )) as [{ source_table: string; source_max_id: number }[], unknown];
  return Object.fromEntries(
    rows.map((row) => [row.source_table, Number(row.source_max_id)]),
  );
}
...
async function recordRowsWatermark(
  conn: PoolConnection,
  table: IdTable,
  rows: { id: number }[],
): Promise<void> {
...
async function main() {
  ...
  if (checkpoint) {
    console.log(
      `Tail import: rows with source SQLite id greater than checkpoint`,
    );
  }
  ...
  if (options.delayMs > 0) {
    console.log(
      `Throttle: ${options.delayMs}ms after each MySQL insert batch/query`,
    );
  }
  ...
  if (options.resumeFromMysqlWatermarks) {
    ...
    console.log(
      `Resume import: loaded ${Object.keys(checkpoint).length} source watermarks from MySQL`,
    );
  }
  ...
  try {
    ...
    // ── provinces ────────────────────────────────────────────────────────────
    {
      ...
      const rowsToInsert = rows.filter((r) =>
        shouldInsertSourceRow("provinces", r.id),
      );
      console.log(
        `provinces: ${rowsToInsert.length} rows${checkpoint ? ` (${rows.length} mapped)` : ""}`,
      );
      ...
      // ── province_resources ───────────────────────────────────────────────
      {
        ...
        await batchInsert(
          ...
          rs.map((r) => [
            ...
            r.total_pop ?? null,
            r.max_pop ?? null,
            r.free_specialist_credits ?? null,
            r.free_building_credits ?? null,
            r.source,
            r.saved_by,
            r.accuracy,
            r.received_at,
      ...
      // ── province_status ──────────────────────────────────────────────────
      {
        type R = {
          ...
          key_hash: string | null;
          plagued: number | null;
          overpopulated: number | null;
          overpop_deserters: number | null;
      ...
      // ── province_effects ─────────────────────────────────────────────────
      {
        ...
        await batchInsert(
          ...
          rs.map((r) => [
            ...
            r.effect_kind,
            r.duration_text,
            r.remaining_ticks ?? null,
            r.effectiveness_percent ?? null,
            r.source,
            r.saved_by,
            r.received_at,
          ]),
      ...
      // ── intel_partitions ─────────────────────────────────────────────────
      {
        ...
        const rs = sqlite
          .prepare("SELECT * FROM intel_partitions")
          .all() as R[];
      ...
      // ── military_intel + som_armies ──────────────────────────────────────
      {
        ...
        const milRows = selectRows<MilRow>(
          sqlite,
          "military_intel",
          "id",
          false,
        );
        const milRowsToInsert = milRows.filter((r) =>
          shouldInsertSourceRow("military_intel", r.id),
        );
        const armyRows = selectRows<ArmyRow>(
          sqlite,
          "som_armies",
          "military_intel_id, id",
        );
        console.log(
          `military_intel: ${milRowsToInsert.length} rows${checkpoint ? ` (${milRows.length} mapped)` : ""}, som_armies: ${armyRows.length} rows`,
        );
        ...
        await batchInsert(
          ...
          armyRows.map((r) => [
            milMap.get(r.military_intel_id)!,
            r.army_type,
            r.generals,
            r.soldiers,
            r.off_specs,
      ...
      // ── survey_intel + survey_buildings ──────────────────────────────────
      {
        ...
        type BldgRow = {
          id: number;
          survey_intel_id: number;
          building: string;
          built: number;
          in_progress: number;
        };
        const surveyRows = selectRows<SurveyRow>(
          sqlite,
          "survey_intel",
          "id",
          false,
        );
        const surveyRowsToInsert = surveyRows.filter((r) =>
          shouldInsertSourceRow("survey_intel", r.id),
        );
        const bldgRows = selectRows<BldgRow>(
          sqlite,
          "survey_buildings",
          "survey_intel_id, id",
        );
        console.log(
          `survey_intel: ${surveyRowsToInsert.length} rows${checkpoint ? ` (${surveyRows.length} mapped)` : ""}, survey_buildings: ${bldgRows.length} rows`,
        );
          ...
          const id = shouldInsertSourceRow("survey_intel", r.id)
            ? await insertOne(
                ...
                [
                  mysqlProvinceId,
                  r.key_hash,
                  r.thievery_effectiveness ?? null,
                  r.thief_prevent_chance ?? null,
                  r.castles_effect ?? null,
                  r.source,
                  r.saved_by,
                  r.accuracy,
                  r.received_at,
        ...
        await batchInsert(
          ...
          bldgRows
            .filter((r) => newSurveyIds.has(r.survey_intel_id))
            .map((r) => [
              surveyMap.get(r.survey_intel_id)!,
              r.building,
              r.built,
              r.in_progress,
            ]),
      ...
      // ── sos_intel + sos_sciences ─────────────────────────────────────────
      {
        ...
        const sosRowsToInsert = sosRows.filter((r) =>
          shouldInsertSourceRow("sos_intel", r.id),
        );
        const sciRows = selectRows<SciRow>(
          sqlite,
          "sos_sciences",
          "sos_intel_id, id",
        );
        console.log(
          `sos_intel: ${sosRowsToInsert.length} rows${checkpoint ? ` (${sosRows.length} mapped)` : ""}, sos_sciences: ${sciRows.length} rows`,
        );
        ...
        await batchInsert(
          ...
          sciRows
            .filter((r) => newSosIds.has(r.sos_intel_id))
            .map((r) => [
              sosMap.get(r.sos_intel_id)!,
              r.science,
              r.books,
              r.effect,
            ]),
      ...
      // ── rob_ops ──────────────────────────────────────────────────────────
      {
        ...
        await batchInsert(
          ...
          rs.map((r) => [
            ...
            r.thieves,
            r.stealth,
            r.troops_assassinated ?? null,
            r.kidnapped ?? null,
            r.acres_burned ?? null,
            r.effect_duration ?? null,
            r.saved_by,
            r.received_at,
          ]),
        );
      ...
      // ── attack_ops ───────────────────────────────────────────────────────
      {
        type R = {
          ...
          buildings_survived: number | null;
          specialist_credits: number | null;
          peasants_settled: number | null;
        ...
        await batchInsert(
          ...
          rs.map((r) => [
            ...
            r.target_kingdom,
            r.acres_taken,
            r.buildings_survived,
            r.specialist_credits,
            r.peasants_settled,
            r.massacred,
            r.enemy_killed,
            r.enemy_imprisoned,
    ...
    // ── key_kingdom_bindings ─────────────────────────────────────────────────
    {
      ...
      const rs = sqlite
        .prepare("SELECT * FROM key_kingdom_bindings")
        .all() as R[];
    ...
    // ── kingdom_intel + kingdom_provinces ────────────────────────────────────
    {
      ...
      const kiRowsToInsert = kiRows.filter((r) =>
        shouldInsertSourceRow("kingdom_intel", r.id),
      );
      const kpRows = selectRows<KpRow>(
        sqlite,
        "kingdom_provinces",
        "kingdom_intel_id, id",
      );
      console.log(
        `kingdom_intel: ${kiRowsToInsert.length} rows${checkpoint ? ` (${kiRows.length} mapped)` : ""}, kingdom_provinces: ${kpRows.length} rows`,
      );
        ...
        const id = shouldInsertSourceRow("kingdom_intel", r.id)
          ? await insertOne(
              ...
              [
                ...
                r.honor_rank,
                r.war_target,
                r.their_attitude_to_us,
                r.their_attitude_points,
                r.our_attitude_to_them,
                r.our_attitude_points,
                r.hostility_meter_visible_until,
                r.open_relations_json,
                r.war_doctrines_json,
                r.source,
                r.saved_by,
                r.received_at,
              ],
