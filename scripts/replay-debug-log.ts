...
function usage() {
  throw new Error(
    "Usage: tsx scripts/replay-debug-log.ts <jsonl...> [--types=kingdom,survey,sot,kingdom_news,state,som,train_army,build] [--key-hash=<sha256>] [--assume-key-hash=<sha256> | --assume-key=<raw key>] [--dry-run] [--refresh-metrics]",
  );
}
